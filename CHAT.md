...
### 4.1. Rate limit по `socket.id`, а не по пользователю — HIGH ✅ ИСПРАВЛЕНО
Раньше `isRateLimited(authSocket.id)` считал лимит на **сокет**, и клиент мог открыть много сокетов (вкладок/переподключений) и умножить лимит `10 msg/sec`. Теперь ключ строится по личности отправителя (`senderType:senderId`), поэтому лимит общий для всех соединений одного пользователя.

### 4.2. Блокировка чата (`chatBlocked`) не мешает отправке сообщений — HIGH ✅ ИСПРАВЛЕНО
Раньше `chat-block` ставил флаг `chatBlocked: true`, но `sendMessageHandler` его не проверял — «заблокированный» пациент мог слать `send-message` напрямую через сокет в обход UI. Теперь хэндлер отклоняет сообщение с ошибкой «Чат заблокирован врачом», если `accessResult.appointment.chatBlocked === true` и отправитель — пациент (врач по-прежнему может писать всегда). Доп. запроса к БД нет — используется уже загруженный в `verifyAppointmentAccess` объект.
