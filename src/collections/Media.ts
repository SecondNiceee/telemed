...
/**
 * Условие «файл помечен публичным».
 *
...
/**
 * Страховка: запись консультации не отдаётся по метке visibility, никогда.
 *
 * Полагаться только на visibility оказалось нельзя. Первая миграция создала
 * колонку с DEFAULT 'public', и Postgres проставил это значение всем уже
 * существующим строкам - записи приёмов получили явную метку «публичный», и
 * список /api/media продолжал отдавать их анонимно даже после деплоя.
 *
 * Здесь права не выводятся из данных, которые могли быть испорчены разметкой:
 * все три точки создания записи дают имя `consultation-<id>-<время>.webm`
 * (finalize, finalize-server, recording-chunks), поэтому такие файлы
 * исключаются из общедоступной выборки на уровне SQL.
 *
 * Это второй слой, а не замена visibility: вложения чатов под шаблон не
 * подходят и закрываются именно меткой. Смысл слоя в том, чтобы запись приёма
 * не утекла из-за неверных данных в колонке.
 *
 * Знак % обязателен. Payload дописывает подстановочные знаки сам только для
 * оператора contains (sanitizeQueryValue), а значение not_like уходит в
 * NOT ILIKE как есть - без % получилось бы сравнение с точной строкой
 * 'consultation-', то есть условие не отсеяло бы ни одного файла.
 */
const NOT_A_RECORDING: Where = { filename: { not_like: 'consultation-%' } }

/** Общедоступная выборка: помечено публичным И не является записью приёма. */
const PUBLIC_FILES: Where = { and: [{ or: PUBLIC_FILE_CONDITIONS }, NOT_A_RECORDING] }

/**
 * Публичное + личное: свои приватные файлы поверх общедоступных.
 *
 * Своя запись приёма приходит через `own` (allowedUser / allowedDoctor), то
 * есть по явно выданному праву, а не потому что файл «оказался публичным».
 */
const withOwnFiles = (own: Where): Where => ({ or: [PUBLIC_FILES, own] })
...
const readAccess = async ({ req }: { req: PayloadRequest }): Promise<boolean | Where> => {
  ...
  // Анонимный посетитель и все прочие: только публичные файлы, без записей.
  return PUBLIC_FILES
}
